...
# Q1 - create a smooth estimate of deaths as a function of date
...
# Q3 - 2 or 7, is the loess line significant?
library(broom)
mnist_27$train %>% glm(y ~ x_2, family = "binomial", data = .) %>% tidy()
qplot(x_2, y, data = mnist_27$train)

fit <- mnist_27$train %>%
  loess(as.numeric(y) ~ as.numeric(x_2), data = ., span = 0.4, degree =1)

mnist_27$train %>%
  mutate(smooth = predict(fit, x_2)) %>%
  ggplot() +
